(function () {
    ...
    function init() {
        ...
        // Nuvemshop: botao Comprar usa .js-addtocart e fica dentro de .js-product-form
        const buyBtnSelectors = [
            '.js-addtocart.btn-big',
        ...
        for (const sel of buyBtnSelectors) {
            ...
            if (buyEl) {
                const target = buyEl.closest('.js-product-form') || buyEl.parentElement;
                target.insertBefore(inlineWrapper, buyEl);
                inlinePlaced = true;
                break;
            }
        }
        if (!inlinePlaced) {
            const anySubmit = document.querySelector('form[action*="/cart"] [type="submit"], form input[name="add"]');
